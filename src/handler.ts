...
import {
    APIDataAdapter,
    APIInputSchemas,
    ComplexZodSchemaPart,
    PrimitiveZodSchemaPart,
...
export type SpringReverbHandler<
...
export type PreHandlerMiddleware<Input = void, Ctx = void> = (ctx: {
    parsedInput: Input;
    context: Ctx;
}) => Promise<void>;

export type AfterHandlerMiddleware<
    Input = void,
    Output = void,
    Ctx = void,
> = (ctx: {
...
>(
    ...
    afterHandler: AfterHandlerMiddleware<
        z.infer<Input>,
        z.infer<Output>,
        Context
    >[] = [],
    ...
    const logic = async (
        ...
        const ctx: SpringContext<z.infer<Input>, z.infer<Output>, Context> = {
            rawInput: input,
        };
    ...
    const REST = <RESTInput extends unknown[], RESTOutput>(
        adapter: APIDataAdapter<RESTInput, RESTOutput>,
    ) => {
        const defaultSchema = () => {
            const schema = inputSchema as any as ZodObject;

            const stringFields = Object.entries(schema.shape)
                .filter((x) => x[1].type === "string")
                .map((x) => x[0]);
            const numericFields = Object.entries(schema.shape)
                .filter((x) => x[1].type === "number")
                .map((x) => x[0]);
            const booleanFields = Object.entries(schema.shape)
                .filter((x) => x[1].type === "boolean")
                .map((x) => x[0]);
            const dateFields = Object.entries(schema.shape)
                .filter((x) => x[1].type === "date")
                .map((x) => x[0]);
            ...
            for (const bool of booleanFields) {
                querySchema = querySchema.extend({
            ...
            type Query = PrimitiveZodSchemaPart<
                Input extends ZodObject ? Input : ZodObject
            >;
            type Body = ComplexZodSchemaPart<
                Input extends ZodObject ? Input : ZodObject
            >;

            const objectFields = Object.entries(schema.shape)
                .filter((x) => x[1].type === "object")
                .map((x) => x[0]);
            const arrayFields = Object.entries(schema.shape)
                .filter((x) => x[1].type === "array")
                .map((x) => x[0]);

            const bodySchema = schema.pick(
                // @ts-ignore
                Object.fromEntries(
                    [...objectFields, ...arrayFields].map((x) => [
                        x,
                        true as const,
                    ]),
                ),
            ...
            type ResultSchemas = (Query extends never
                ? {}
                : { querySchema: Query }) &
                (Body extends never ? {} : { bodySchema: Body }) & {
                    response: Output;
                };
        ...
        >(
            schemasGenerator: Input extends ZodObject
                ? APIInputSchemas<Input, QuerySchema, BodySchema>
                : never,
            inputMapping?: (
                x: z.infer<QuerySchema> & z.infer<BodySchema>,
            ) => z.infer<Input>,
        ) => {
            // @ts-ignore
            ...
            const handler = async (...i: RESTInput) => {
                ...
                const resultInput = inputMapping
                    ? inputMapping(
                          inputData as z.infer<QuerySchema> &
                              z.infer<BodySchema>,
                      )
            ...
            return Object.assign(handler, {
                ___api_metadata: { ...schemas, response: outputSchema },
            });
        };
...
export function handlerWithCtx<
    Input extends ZodType,
    Output extends ZodType,
    Context,
>(
    base: {
        ...
        middlewares?: {
            beforeHandler?: Array<
                PreHandlerMiddleware<z.infer<Input>, Context>
            >;
            afterHandler?: AfterHandlerMiddleware<
                z.infer<Input>,
                z.infer<Output>,
    ...
    return (
        getContext: Context | (() => Context) | (() => Promise<Context>),
    ) => {
        return springReverbBase(
            ...
            base.middlewares?.beforeHandler,
            base.middlewares?.afterHandler,
            base.middlewares?.onError,
        );
    };
}

export function handler<Input extends ZodType, Output extends ZodType>(
    base: {
        input: Input;
        output: Output;
        handler: (
...
export function handler(metadata: any, additionalMetadata: any) {
    if (metadata.handlerWithCtx) {
        return (getContext: () => any) => {
            return springReverbBase(
                metadata.input,
                metadata.output,
                { getContext, handler: metadata.handlerWithCtx },
                additionalMetadata?.sourceForErrorDetails,
    ...
    return springReverbBase(
        ...
        metadata.output,
        metadata.getContext
            ? { getContext: metadata.getContext, handler: metadata.handler }
            : metadata.handler,
        additionalMetadata?.sourceForErrorDetails,
        metadata?.middlewares?.beforeHandler,
        metadata?.middlewares?.afterHandler,
